...
import {CalculateDragLimitsRequest} from "../calculate-drag-limits";
...
export class CreateSummaryDragHandler
  ...
  private _collectNodesMoveLimits(hierarchyRoots: MoveNodeOrGroupDragHandler[]): INodeMoveLimitsAndPosition[] {
    ...
    hierarchyRoots.forEach((dragHandler) => {
      ...
      result.push({position: dragHandler.nodeOrGroup._position, ...calculatedLimits});
    });

    return result;
  }

  private _calculateDragLimits(nodeOrGroup: FNodeBase): INodeMoveLimits {
    return this._mediator.execute<IMinMaxPoint>(new CalculateDragLimitsRequest(nodeOrGroup));
  }
