...
function addDependencies(installationState: { hasNewDependencies: boolean }): Rule {
  return (tree: Tree, context: SchematicContext) => {
    context.logger.info('Installing dependencies for @foblex/flow-elk-layout...');
...
function installDependencies(installationState: { hasNewDependencies: boolean }): Rule {
  return (_tree: Tree, context: SchematicContext) => {
    ...
    context.logger.info('ELK.js support is ready.');
    context.logger.info(
      'Next step: provide ElkLayoutEngine and call calculate(...) in your flow component.',
    );
