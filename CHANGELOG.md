...
- **a11y:** every flow now applies ARIA semantics by default — the `f-flow` host gains `role="application"`, nodes/groups/connections gain `role`, `aria-roledescription`, a DOM `id` when they have none, and (for connections) a generated `aria-label`; any attribute you set yourself wins. These are inert attributes: no behavior changes without opting into the keyboard layer via `withA11y()`.
