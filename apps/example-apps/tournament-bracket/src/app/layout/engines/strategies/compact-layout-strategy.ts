...
export class CompactLayoutStrategy implements ITournamentLayoutStrategy {
  public calculate(
    nodes: IFLayoutNode[],
    ...
    const compactGap = Math.round(nodeGap * 0.5);
    const compactLayerGap = Math.round(layerGap * 0.55);
    ...
    const separation = upperNodes.length > 0 ? compactGap * 3 : 0;
    ...
    finalNodes.forEach((node) => {
      const x = hasBracketRounds ? (maxRound + 1) * (nodeWidth + compactLayerGap) : 0;
