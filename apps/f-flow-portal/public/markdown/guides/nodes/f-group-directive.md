...
With the optional [Managed Flow State](./examples/state) plugin, completed move gestures update group records automatically, and `fGroupSizeChange` synchronizes resize geometry without a standalone undo step. Rotation is not captured by managed state v1, so applications using it still persist the final output themselves.
...
[example.html] <<< https://raw.githubusercontent.com/Foblex/f-flow/main/libs/f-examples/nodes/grouping/example.html
[example.ts] <<< https://raw.githubusercontent.com/Foblex/f-flow/main/libs/f-examples/nodes/grouping/example.ts
[example.scss] <<< https://raw.githubusercontent.com/Foblex/f-flow/main/libs/f-examples/nodes/grouping/example.scss
:::
