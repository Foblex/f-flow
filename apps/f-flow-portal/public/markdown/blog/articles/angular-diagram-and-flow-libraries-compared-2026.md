...
- [**ngx-vflow**](https://www.ngx-vflow.org/) ([MIT](https://github.com/artem-mangilev/ngx-vflow/blob/main/LICENSE)) — an array-driven `nodes[]`/`edges[]` model in native Angular. Its current official feature list includes subflows, connection editing, keyboard shortcuts, virtualization, and lazy loading; it is a good candidate when that data-first mental model fits your team. Detailed breakdown: [ngx-vflow vs Foblex Flow](./docs/ngx-vflow-vs-foblex-flow).
