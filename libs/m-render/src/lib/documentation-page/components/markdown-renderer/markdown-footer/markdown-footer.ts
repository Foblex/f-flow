...
export class MarkdownFooter {
  ...
  private readonly _url = toSignal(this._router.events.pipe(
    startWith(null),
  ...
  protected readonly editLink = computed<string | undefined>(() => {
    ...
    return nav.editLink.pattern + this._currentLink() + '.md';
  });

  protected readonly previousLink = computed<IMarkdownFooterLink | undefined>(() => {
    const prevNext = new GetPreviousNextPageNavigationHandler(this._provider)
      .handle(new GetPreviousNextPageNavigationRequest(this._currentLink()));
  ...
  private _normalizeLink(item?: IMarkdownFooterLink): IMarkdownFooterLink | undefined {
    ...
    const joined = (prefix ? `${prefix}/${item.link}` : item.link)
      .replace(/\/{2,}/g, '/');
