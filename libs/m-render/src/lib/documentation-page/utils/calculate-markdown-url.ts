...
export function calculateMarkdownUrl(
  markdown: string,
  ...
  const relativePath = markdownPaths?.[markdown] ?? markdown;
  let url = docsDir + relativePath;
...
function _isMarkdownExist(id: string, navigation: INavigationGroup[]): boolean {
  return !!navigation.find((x) => x.items.some((i: { link: string }) => i.link === id));
}
